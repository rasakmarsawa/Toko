...
--
-- Host: 127.0.0.1
-- Waktu pembuatan: 03 Agu 2022 pada 17.47
...
--
DELIMITER $$
CREATE TRIGGER `getTransactionId` BEFORE INSERT ON `transaction` FOR EACH ROW BEGIN
DECLARE a integer;
SET a = (select max(transaction.transaction_id) from transaction where transaction.supplier_id = new.supplier_id);

IF a IS null THEN
	SET new.transaction_id = 1;
ELSE
	SET new.transaction_id = a+1;
END IF;
END
$$
DELIMITER ;
